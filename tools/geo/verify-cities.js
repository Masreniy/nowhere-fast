 ...
 *
 * ЕСЛИ ХЭШ НЕ СОШЁЛСЯ, первым делом смотри на нормализацию Unicode, а не на
 * опечатку. Пересказ текста через модель канонически эквивалентен, но не
 * побайтово равен: у Каунсил-Блафс в написании на хинди предсоставленный
 * символ U+095E (फ़) приехал в базу разложенным на U+092B U+093C. Глазами
 * это один и тот же знак, длина строки та же, а байты другие. Поймал ровно
 * этот хэш — без него расхождение осталось бы в базе навсегда.
 *
 * Отсюда правило: сверка НЕ нормализует строки перед сравнением. Стоит ей
 * привести обе стороны к NFC — и она перестанет видеть именно тот класс
 * порчи, ради которого написана.
 *
