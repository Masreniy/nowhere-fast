...
NF.api = (function () {
    ...
    /**
     * Авторские маршруты города вместе с днями и активностями.
     *
     * Один запрос вместо лестницы циклов. Раньше здесь было N+1: на каждый
     * авторский маршрут отдельный запрос дней, на каждый день — запрос активностей.
     * Авторский маршрут на 5 дней означал 11 обращений к базе.
     *
    ...
    async function listRoutes(cityId) {
        ...
        if (error) {
            console.warn(
                'Nowhere Fast: вложенная выборка авторских маршрутов не сработала. ' +
     ...
     * в произвольном порядке, и дни авторского маршрута перемешиваются незаметно.
